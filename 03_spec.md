...
実装を開始する前に、以下を必ず実施すること。

```
Step A: 対象コードの把握
  - 変更対象ファイルを読み、既存の実装・テストを把握する
  - 関連ファイル（呼び出し元・呼び出し先）も確認する
  - 現在のテスト件数を記録する

Step B: テスト計画の策定・合意
  - 変更箇所の一覧
  - 追加・変更するテストケース一覧（正常系・異常系・境界値）
  - 既存テストへの影響範囲
  → ユーザーに確認を取ってから実装へ

Step C: 実装
  - 関数・モジュール単位で小さく実装し、都度テストを回す
  - 既存の設計・命名規則・スタイルに従う
...
Step E: コードレビュー
  - simplify または critic サブエージェントでレビュー
  - セキュリティ・安全装置・後方互換性を確認
