...
> **注意：** このファイル（02_claude-md.md）自体は Phase 2 の手順書であり、
> 作成するCLAUDE.mdそのものではない。以下のサイズ制約は「あなたが作成するCLAUDE.md」に適用される。
...
```
ツールで取得可能な情報はCLAUDE.mdに書くな。
「git log で分かること」「ls で分かること」「grep で分かること」は不要。

❌ ファイル一覧・テスト件数・最近のコミット → ツールで動的に取得すべき
❌ 長い依存関係リスト → package.json / pyproject.toml を読めば分かる
❌ コードの詳細な説明 → コードを直接読めば分かる

原則：「ツールで取得できるならCLAUDE.mdには書かない」
```
...
```
CLAUDE.mdに以下を追加すると、毎セッション開始時の品質が安定する：

## Session Startup
1. TODO.md を読んで In Progress セクションを確認
2. 現状を報告してから「次に何をすべきか？」を確認
3. 人間の承認を得てから作業開始

→ CCが勝手に作業を始めることを防ぎ、優先順位のズレを早期発見できる
```

---

## ナレッジ分離パターン（Bot/AI系プロジェクト向け）

```
Bot・AI系プロジェクトでは、ドメイン知識をコードから分離すると
人格・トーン・禁止ワードなどの変更がコード変更なしで可能になる。

knowledge/
├── persona.txt       # ボットの人格定義
├── post_patterns.txt # 出力パターン・トーン
