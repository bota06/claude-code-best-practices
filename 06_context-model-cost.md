...
### コンテキスト肥大化防止ルール（実践で確認済み）

```
以下のルールを守らないとセッションがクラッシュする（実測値ベース）。
特に小〜中規模プロジェクトでも、ファイル数が増えると容易に発生する。

□ Glob：浅いパターンのみ使え
  ✅ project/*          （直下のみ）
  ❌ project/**/*       （再帰的 → 大量のファイルパスがコンテキストに入る）

□ Read：1ファイルずつ読め
  ✅ 1ファイル → 内容確認 → 次のファイル
  ❌ 複数ファイルを一度に Read（コンテキストが一気に膨張する）

□ 大きいファイルはGrepで絞ってからReadせよ
  ✅ grep "target_function" file.py → 該当行付近だけ Read
  ❌ 200行超のファイルを全文 Read

□ 1ステップ = 1ツール呼び出し を意識せよ
  → 並列ツール呼び出しはコンテキスト爆発の原因になる
  → 特に調査フェーズでは逐次実行が安全
```
